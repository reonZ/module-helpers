...
type WithSocketOptionsRequired<TOptions extends Record<string, any>, TRequired = RequiredFieldsOnly<TOptions>> = TRequired extends Record<infer TKey, any> ? {
    [k in TKey]: TRequired[k] extends ClientDocument ? TRequired[k] | string : TRequired[k] extends Record<string, any> ? WithSocketOptions<TRequired[k]> : TRequired[k];
} : never;
type WithSocketOptionsPartial<TOptions extends Record<string, any>, TPartial = PartialFieldsOnly<TOptions>> = TPartial extends Partial<Record<infer TKey, any>> ? {
    [k in TKey]?: NonNullable<TPartial[k]> extends ClientDocument ? TPartial[k] | string : NonNullable<TPartial[k]> extends Record<string, any> ? WithSocketOptions<TPartial[k]> : TPartial[k];
} : never;
