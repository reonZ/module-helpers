import { ActorPF2e, DamageType, PersistentDamagePF2e } from "foundry-pf2e";
import { getDamageRollClass } from "../classes";
import { DAMAGE_TYPE_ICONS, damageDiceIcon } from "./damage";
import { htmlClosest, htmlQuery, htmlQueryAll } from "./dom";
...
class PersistentDamageDialog extends Application<PersistentDamageDialogOptions> {
    ...
    #reportFormulaValidity(formula: string, input: HTMLInputElement | null): boolean {
        ...
        if (isValid) {
            ...
            input.setCustomValidity(
                game.i18n.localize("PF2E.Item.Condition.PersistentDamage.Dialog.Invalid")
            );
    ...
    protected override _injectHTML($html: JQuery<HTMLElement>): void {
        ...
        const existing = this.options.editing
            ? htmlQuery(html, `[data-id=${this.options.editing}] .formula`)
            : null;
