import { ActorPF2e, CoinsPF2e } from "foundry-pf2e";

function hasSufficientCoins(actor: ActorPF2e, value: CoinsPF2e, byValue: boolean = true) {
    ...
    if (byValue) {
        ...
        if (valueToRemoveInCopper % 10 > actorCoins.cp) {
            ...
            const extraCopper =
                Math.min(valueToRemoveInCopper / 10, Math.trunc(newCopper / 10)) * 10;
        ...
        if ((valueToRemoveInCopper / 10) % 10 > actorCoins.sp) {
            ...
            const extraSilver =
                Math.min(valueToRemoveInCopper / 100, Math.trunc(newSilver / 10)) * 10;
