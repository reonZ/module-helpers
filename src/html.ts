...
function createFormData(
    ...
    { expand = false, disabled, readonly }: FormData = {}
): Record<string, unknown> | null {
    const form = html instanceof HTMLFormElement ? html : htmlQuery(html, "form");
    if (!form) return null;

    interface FormDataExtender {
        prototype: FormDataExtended;
        new (form: HTMLFormElement, options?: FormDataExtendedOptions): FormDataExtended;
    }

    const data = R.pipe(
        new (FormDataExtended as FormDataExtender)(form, { disabled, readonly }).object,
